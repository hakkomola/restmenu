...
<form method="post" enctype="multipart/form-data" id="editForm">
    ...
    <div class="mb-3">
        ...
        <input type="file" id="newImage" multiple class="form-control">
    </div>
    ...
    <div class="mb-3">
        ...
        <div class="row" id="imageContainer">
            <?php foreach ($images as $img):
                // DB’deki URL zaten uploads/ ile başlıyorsa ../ eklemiyoruz, yoksa ekliyoruz
                $imgUrl = $img['ImageURL'];

            ?>
                <div class="col-md-3 text-center mb-3" data-id="<?= $img['MenuImageID'] ?>">
                    <img src="../<?= htmlspecialchars($imgUrl) ?>" class="img-fluid rounded mb-2" style="max-height:150px; object-fit:cover;">
                    <button type="button" class="btn btn-sm btn-danger delete-btn" data-id="<?= $img['MenuImageID'] ?>">Sil</button>
                </div>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    .then(data => {
        if(data.success){
            data.images.forEach(img => {
                ...
                div.innerHTML = `<img src="../${img.url}" class="img-fluid rounded mb-2" style="max-height:150px; object-fit:cover;">
                                 <button type="button" class="btn btn-sm btn-danger delete-btn" data-id="${img.id}">Sil</button>`;
